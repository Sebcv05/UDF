...
CONVERGE_ONLOAD(spray_env, IN(CONVERGE_VOID))
{
   ...
   CONVERGE_variable_register("user_lifetime", CONVERGE_DOUBLE, DEFAULT_PARCEL_VARIABLE_SETTINGS, END_ARG_LIST);
   ...
   USER_LIFETIME = CONVERGE_lagrangian_field_id("user_lifetime");
...
void load_user_cloud(struct ParcelCloud *parcel_cloud_loc, CONVERGE_cloud_t c)
{
   ...
   parcel_cloud_loc->user_lifetime = (CONVERGE_precision_t *)CONVERGE_cloud_get_field_data(c, USER_LIFETIME);
