...
void spray_evap_cell(CONVERGE_cloud_t cloud)
{
   ...
   const CONVERGE_precision_t min_spray_temp          = 185.0;
   const CONVERGE_precision_t min_spray_recovery_temp = 194.0;
   ...
   ////////////////
      ...
      parcel_cloud.v_sh[i_pc] = 2.0 + 0.6 * sqrt(parcel_cloud.rey_num[i_pc]) * (CONVERGE_cbrt(sc_num));
         ...
         fprintf(fp1, "%i    %i    %f    %e    %e    %e\n", parcel_cloud.cloud_index[0], parcel_cloud.parcel_index[0], parcel_cloud.temp[0], parcel_cloud.radius[0], parcel_cloud.user_lifetime[0]+parcel_cloud.lifetime[0],vmag);
         fclose(fp1);
         // ******************************************************************************************************//
         ...
         while((inner_iter_flag == 1) && (inner_iter < max_inner_iter))
         {
            ...
            else
            {
               if(spray_evap_flag == 1)
               {
                  ...
                  // if( evap_flag_flash_boiling==1 ) //Leave off for Frossling Model 
                  // {
                  // if( user_dT_sh > 0.002 )
                  // {
