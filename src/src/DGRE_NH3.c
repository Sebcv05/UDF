...
void DGRE_NH3(struct ParcelCloud* old_parcel_cloud,CONVERGE_index_t p_idx,CONVERGE_precision_t global_density)
{
            ...
            CONVERGE_precision_t P_sat;
             Saturation_PressureNH3(old_parcel_cloud->temp[p_idx],&P_sat);
            CONVERGE_precision_t rho_b = bubble_densityNH3(P_sat,old_parcel_cloud->temp[p_idx]);
            ...
            CONVERGE_precision_t sound_speed = 471.706; // approx number for NH3 at 360 K 2.4 bar (perhaps should consider some function for this as it varies a fair amount with P,T) https://doi.org/10.1016/j.jct.2007.07.002
