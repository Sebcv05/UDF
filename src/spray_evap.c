...
// Static SILO objects checked out in ONLOAD function
static CONVERGE_cell_double_t* density = NULL;
static CONVERGE_cell_double_t* mol_conductivity = NULL;
static CONVERGE_cell_double_t* csubp = NULL;
static CONVERGE_cell_double_t* csubv = NULL;
static CONVERGE_cell_double_t* mol_viscosity = NULL;
static CONVERGE_cell_double_t* temperature = NULL;
static CONVERGE_cell_double_t* volume = NULL;
static CONVERGE_cell_double_t* pressure = NULL;
static CONVERGE_cell_double_t* sensible_sie = NULL;
static CONVERGE_cell_double_t* species_massfrac = NULL;
static CONVERGE_cell_double_t* species_massfrac_tm1 = NULL;
static CONVERGE_cell_double_t* density_tm1 = NULL;
static CONVERGE_cell_double_t* src_ex_density = NULL;
static CONVERGE_cell_double_t* src_ex_sensible_sie= NULL;
static CONVERGE_cell_double_t* src_ex_species_massfrac= NULL;
static CONVERGE_cell_double_t* ecfm3z_src_unburned_enth_evap_ex= NULL;
static CONVERGE_cell_double_t* src_rif_fuel_evap_ex= NULL;
static CONVERGE_cell_double_t* ecfm_temp_evap_ex= NULL;
static CONVERGE_cell_short_t* region_index = NULL;
static CONVERGE_cell_vec3_t* velocity_object = NULL;
...
void custom_spray_evap()
{
   ...
   const int stream_index = CONVERGE_current_stream_index();
   CONVERGE_mesh_t mesh = CONVERGE_retrieve_mesh(stream_index);
   ...
   // printf("\n spray_evap.c L131");
   CONVERGE_vec3_t* velocity = velocity_object[stream_index]->data;
   ...
   num_total_species  = CONVERGE_species_num_tot(sp);
   ...
   global_density                   = density[stream_index]->data;
   global_mol_cond                  = mol_conductivity[stream_index]->data;
   global_csubp                     = csubp[stream_index]->data;
   global_csubv                     = csubv[stream_index]->data;
   global_mol_viscosity             = mol_viscosity[stream_index]->data;
   global_temperature               = temperature[stream_index]->data;
   global_volume                    = volume[stream_index]->data;
   global_pressure                  = pressure[stream_index]->data;
   global_sensible_sie              = sensible_sie[stream_index]->data;
   global_species_massfrac_tm1      = species_massfrac_tm1[stream_index]->data;
   global_species_massfrac          = species_massfrac[stream_index]->data;
   global_src_species_ex            = src_ex_species_massfrac[stream_index]->data;
   global_density_tm1               = density_tm1[stream_index]->data;
   global_src_ex_density            = src_ex_density[stream_index]->data;
   global_src_sie_ex                = src_ex_sensible_sie[stream_index]->data;
   global_src_unburned_enth_evap_ex = ecfm3z_src_unburned_enth_evap_ex[stream_index]->data;
   global_src_rif_fuel_evap_ex      = src_rif_fuel_evap_ex[stream_index]->data;
   global_ecfm_temp_evap_ex         = ecfm_temp_evap_ex[stream_index]->data;
   global_region_index              = region_index[stream_index]->data;
...
void spray_evap_cell(CONVERGE_cloud_t cloud)
{
   ...
   const CONVERGE_precision_t min_spray_temp          = 195.4;             //Freezing point of ammonia at 1bar
   const CONVERGE_precision_t min_spray_recovery_temp = 200.0;             //Slightly above this 
   ...
   cell_mass = global_density[node_index] * global_volume[node_index];
   ...
   for(CONVERGE_index_t i_pc = CONVERGE_iterator_first(pc_it); i_pc != -1; i_pc = CONVERGE_iterator_next(pc_it))
   {
      ...
      // parcel_cloud.r_bubble[i_pc] = 1.0000e-3;
      parcel_cloud.thermal_breakup_flag[i_pc] = 1;
   }
         // printf("\n spray_evap.c start of printer function");
         ...
         fprintf(fp1, "%08f    %08f    %f    %e    %e    %e\n", parcel_cloud.cloud_index[0], parcel_cloud.user_parcel_index[0], parcel_cloud.temp[0], parcel_cloud.radius[0], parcel_cloud.lifetime[0],vmag);
         fclose(fp1);
         //******************************************************************************************************//
   ...
   //
   //******************************************************************************************************//
   const CONVERGE_precision_t local_volume = global_volume[node_index];
   ...
   while(fabs(delta_gas_temp) > 1.0 && fabs(temp_gas - temp_gas_iterm1) > 1.0)
   {
      ...
      for(CONVERGE_index_t i_pc = CONVERGE_iterator_first(pc_it); i_pc != -1; i_pc = CONVERGE_iterator_next(pc_it))
      {
         ...
         while((inner_iter_flag == 1) && (inner_iter < max_inner_iter))
         {
            ...
            {
            printf("\nspray_evap.c Recovering as Tdrop at freezing point, Tdrop = %f, Tdrop_tm1 = %f \n",tdrop,tdrop_starm1);
   ...
   for(CONVERGE_index_t i_pc = CONVERGE_iterator_first(pc_it); i_pc != -1; i_pc = CONVERGE_iterator_next(pc_it))
   {
      ...
      //  heat of vaporization should be added to the evaporated liquid
      denth += (enth - hvap) - mass_star * (enthalpy_liquid1 - enthalpy_liquid);
      if(CONVERGE_get_int("inputs.energy_output_flag"))
      {
...
{
   CONVERGE_retrieve_cell_double_streams("density", CONVERGE_CURRENT, &density);
   CONVERGE_retrieve_cell_double_streams("mol_conductivity", CONVERGE_CURRENT, &mol_conductivity);
   CONVERGE_retrieve_cell_double_streams("csubp", CONVERGE_CURRENT, &csubp);
   CONVERGE_retrieve_cell_double_streams("csubv", CONVERGE_CURRENT, &csubv);
   CONVERGE_retrieve_cell_double_streams("mol_viscosity", CONVERGE_CURRENT, &mol_viscosity);
   CONVERGE_retrieve_cell_double_streams("temperature", CONVERGE_CURRENT, &temperature);
   CONVERGE_retrieve_cell_double_streams("volume", CONVERGE_CURRENT, &volume);
   CONVERGE_retrieve_cell_double_streams("pressure", CONVERGE_CURRENT, &pressure);
   CONVERGE_retrieve_cell_double_streams("sensible_sie", CONVERGE_CURRENT, &sensible_sie);
   CONVERGE_retrieve_cell_double_streams("species_massfrac", CONVERGE_CURRENT, &species_massfrac);
   CONVERGE_retrieve_cell_double_streams("species_massfrac", CONVERGE_TM1, &species_massfrac_tm1);
   CONVERGE_retrieve_cell_double_streams("density", CONVERGE_TM1, &density_tm1);
   CONVERGE_retrieve_cell_double_streams("density", CONVERGE_SRCEX, &src_ex_density);
   CONVERGE_retrieve_cell_double_streams("sensible_sie", CONVERGE_SRCEX, &src_ex_sensible_sie);
   CONVERGE_retrieve_cell_double_streams("species_massfrac", CONVERGE_SRCEX, &src_ex_species_massfrac);
   CONVERGE_retrieve_cell_double_streams("ecfm3z_src_unburned_enth_evap_ex", CONVERGE_CURRENT, &ecfm3z_src_unburned_enth_evap_ex);
   CONVERGE_retrieve_cell_double_streams("src_rif_fuel_evap_ex", CONVERGE_CURRENT, &src_rif_fuel_evap_ex);
   CONVERGE_retrieve_cell_double_streams("ecfm_temp_evap_ex", CONVERGE_CURRENT, &ecfm_temp_evap_ex);
   CONVERGE_retrieve_cell_short_streams("region_index", CONVERGE_CURRENT, &region_index);
   CONVERGE_retrieve_cell_vec3_streams("velocity", CONVERGE_CURRENT, &velocity_object);
...
{
   CONVERGE_release_cell_double_streams(&density);
   CONVERGE_release_cell_double_streams(&mol_conductivity);
   CONVERGE_release_cell_double_streams(&csubp);
   CONVERGE_release_cell_double_streams(&csubv);
   CONVERGE_release_cell_double_streams(&mol_viscosity);
   CONVERGE_release_cell_double_streams(&volume);
   CONVERGE_release_cell_double_streams(&temperature);
   CONVERGE_release_cell_double_streams(&pressure);
   CONVERGE_release_cell_double_streams(&sensible_sie);
   CONVERGE_release_cell_double_streams(&species_massfrac);
   CONVERGE_release_cell_double_streams(&species_massfrac_tm1);
   CONVERGE_release_cell_double_streams(&density_tm1);
   CONVERGE_release_cell_double_streams(&src_ex_density);
   CONVERGE_release_cell_double_streams(&src_ex_sensible_sie);
   CONVERGE_release_cell_double_streams(&src_ex_species_massfrac);
   CONVERGE_release_cell_double_streams(&ecfm3z_src_unburned_enth_evap_ex);
   CONVERGE_release_cell_double_streams(&src_rif_fuel_evap_ex);
   CONVERGE_release_cell_double_streams(&ecfm_temp_evap_ex);
   CONVERGE_release_cell_short_streams(&region_index);
   CONVERGE_release_cell_vec3_streams(&velocity_object);
